...
class AdminController extends Controller
{
    ...
    public function leaderboardSemi()
    {
        $teams = Team::with('semiStatistic')
            ->orderByDesc(function ($query) {
                $query->select('score')
                    ->from('semi_statistics')
                    ->whereColumn('semi_statistics.id_team', 'teams.id')
                    ->limit(1);
            })
            ->get();
        return view('admin.leaderboardSemi', [
            'teams' => $teams,
            'title' => 'Leadeboard Semifinal',
        ]);
    }
