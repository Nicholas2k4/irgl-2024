...
@section('body')
    ...
    <div
                    ...
                    <tbody class="text-gray-700 divide-y divide-gray-200">
                        @foreach ($jadwal as $data)
                            <tr>
                                ...
                                <td class="py-2 px-4 text-center">
                                    {{ Carbon\Carbon::parse($data->start_time)->isoFormat('HH:mm') }}</td>
                                <td class="py-2 px-4 text-center">
                                    {{ Carbon\Carbon::parse($data->end_time)->isoFormat('HH:mm') }}</td>
                                <td class="py-2 px-4 text-center">
        ...
        <div
            ...
            <h1 class="text-3xl font-bold mb-8 dark:text-white text-center">Reschedule Requests</h1>
            <div class="container mx-auto">
                <div class="overflow-auto max-h-[500px]">
                            ...
                            @foreach ($teamResched as $data)
                                @if ($data->resched_approval == 2)
                                    <tr>
                                        ...
                                        <td class="py-2 px-4 text-center">
                                            <a href="{{ asset('storage/uploads/' . basename($data->link_bukti_resched)) }}"
                                        ...
                                        <td class="py-2 px-4 text-center">
                                            ...
                                            @elseif ($data->resched_approval == 1)
                                                <p class="bg-green-200 text-green-700 font-bold py-2 px-4 rounded">
                                                    Approved</p>
                                            @else
                                            ...
                                            <form action="{{ route('admin.jadwal.reject', $data->id) }}" method="POST"
                                                ...
                                                <form action="{{ route('admin.jadwal.approve', $data->id) }}"
                                                    method="POST" class="hidden" id='formApproveResched'>
                                                    @csrf
                                                </form>
                                                <button
                                                    class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded"
                                                    data-team="{{ $data->nama }}"
                                                    id='buttonApproveResched'>Approve</button>

                                                <form action="{{ route('admin.jadwal.reject', $data->id) }}" method="POST"
                                                    class="hidden" id='formRejectResched'>
                                                    @csrf
                                                </form>
                                                <button
                                                    class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
                                                    data-team="{{ $data->nama }}"
                                                    id="buttonRejectResched">Reject</button>
                                            @endif
                                        </td>
                                @endif
                                </tr>
...
@section('script')
    <script>
        ...
        $(document).on("click", "#buttonApproveResched", function() {
            ...
            Swal.fire({
                title: 'Approve reschedule tim ' + teamName + '?',
                ...
                if (result.isConfirmed) {
                    var form = $(this).siblings('#formApproveResched');
                    form.submit();
                }
            });
        });

        $(document).on("click", "#buttonRejectResched", function() {
            var teamName = $(this).data('team');
            Swal.fire({
                title: 'Reject reschedule tim ' + teamName + '?',
                text: "Anda tidak dapat mengembalikan tindakan ini!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                ...
                if (result.isConfirmed) {
                    var form = $(this).siblings('#formRejectResched');
                    form.submit();
                }
            });
        });
