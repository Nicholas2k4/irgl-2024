...
<div class="bg-[rgba(0,0,0,0.5)] backdrop-blur-lg shadow-[0_0_20px_rgba(255,255,255,0.1)] md:w-[420px] w-5/6 p-10 rounded-[20px] text-center m-8 relative">
    ...
    <form action="{{ route('register.show.step.three') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-2 flex flex-col items-left">
            ...
            <textarea id="alamat-anggota2-id" name="alamatAnggota2" autocomplete="street-address" required class="flex-1 bg-[rgba(255,255,255,0.1)] text-sm md:text-base text-white shadow-[0_0_10px_rgba(255,255,255,0.1)] p-2.5 rounded-[5px] border-none">{{ old('alamatAnggota2', session('step3.alamatAnggota2')) }}</textarea>
        </div>
        ...
        <div class="mb-2 flex flex-col items-left">
            <label for="file-anggota2-id" class="text-sm md:text-base text-white block text-left mb-2">Foto kartu siswa atau surat pernyataan siswa aktif (Max 5MB)</label>
